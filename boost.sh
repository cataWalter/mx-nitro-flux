...
if [ -x "/etc/init.d/lightdm" ];
then update-rc.d lightdm disable || true;
fi
if [ -x "/etc/init.d/nodm" ];
then update-rc.d nodm enable || true;
fi
...
then sed -i 's/^PERCENT=.*/PERCENT=60/' /etc/default/zramswap;
fi
if [ -f /etc/default/earlyoom ];
then sed -i 's/^EARLYOOM_ARGS=.*/EARLYOOM_ARGS="-m 5 -s 5 --avoid ^(Xorg|nodm|fluxbox)$"/' /etc/default/earlyoom;
fi
...
do
if [ -f "$target" ];
then dpkg-divert --add --rename --divert "$target.disabled" "$target";
fi
done
sed -i 's/^[3-6]:23:respawn:/
CURRENT_HOSTNAME=$(hostname)
                 if ! grep -q "127.0.1.1.*$CURRENT_HOSTNAME" /etc/hosts;
then echo "127.0.1.1 $CURRENT_HOSTNAME" >> /etc/hosts;
fi
log_section "11/15 Firefox Optimization"
FF_DIR="$USER_HOME/.mozilla/firefox"
       if [ -d "$FF_DIR" ];
then
find "$FF_DIR" -maxdepth 1 -type d -name "*.default*" | while read -r PROFILE_DIR;
do
...
for item in conkystart mx-welcome picom compton clipman;
do
sed -i "s/^$item/
done
sed -i 's|^/usr/lib/policykit-1-gnome/.*|
read -r -d '' OPT_BLOCK << EOM || true
                        export NO_AT_BRIDGE=1
                                setxkbmap $KEYBOARD_LAYOUT &
                                lxpolkit &
                                EOM
                                if ! grep -q "GOLD MASTER" "$STARTUP_FILE";
then
...
apt clean
log_section "OPTIMIZATION COMPLETE (v8.6)"
