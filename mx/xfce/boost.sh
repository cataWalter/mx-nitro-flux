...
LOG_FILE="/var/log/mx_xfce_optimization_v8.6_$(date +%Y%m%d_%H%M%S).log"
touch "$LOG_FILE"
...
# Protect Core XFCE packages
apt-mark manual mx-apps-xfce xfce4-panel xfwm4 xfdesktop4 thunar mx-updater cleanup-notifier-mx
...
# Ensure nodm knows to start XFCE
...
# Added fluxbox removal to this list, removed xfce core components from removal
apt purge \
    preload \
    ...
    conky-all mx-conky \
    fluxbox mx-fluxbox mx-apps-fluxbox \
    xfce4-clipman clipman xfce4-dict xfce4-notes* \
...
if [ -f /etc/default/earlyoom ]; then sed -i 's/^EARLYOOM_ARGS=.*/EARLYOOM_ARGS="-m 5 -s 5 --avoid ^(Xorg|nodm|xfce4-session)$"/' /etc/default/earlyoom; fi
...
log_section "12/15 XFCE Startup & Autostart"
AUTOSTART_DIR="$USER_HOME/.config/autostart"
mkdir -p "$AUTOSTART_DIR"

# 1. Disable Compositor (Aggressive Speed)
# Note: Requires xfconfd to be running usually, but we try modifying xml directly if off
# We will use autostart to enforce settings on first login
cat <<EOF > "$AUTOSTART_DIR/optimize_xfce.desktop"
[Desktop Entry]
Type=Application
Name=XFCE Optimization
Exec=sh -c "xfconf-query -c xfwm4 -p /general/use_compositing -n -t bool -s false; xfconf-query -c xfce4-session -p /general/SaveOnExit -n -t bool -s false"
Terminal=false
StartupNotify=false
EOF

# 2. Set Keyboard Layout
cat <<EOF > "$AUTOSTART_DIR/set_keyboard.desktop"
[Desktop Entry]
Type=Application
Name=Set Keyboard
...
# 3. Enable LXPolkit (since we removed gnome-polkit/others)
cat <<EOF > "$AUTOSTART_DIR/lxpolkit.desktop"
[Desktop Entry]
Type=Application
Name=LXPolkit
Exec=lxpolkit
...
# 4. Disable standard XFCE bloat in autostart by masking
for item in xfce4-clipman conky mx-welcome blueman; do
    if [ -f "/etc/xdg/autostart/$item.desktop" ]; then
        cp "/etc/xdg/autostart/$item.desktop" "$AUTOSTART_DIR/$item.desktop"
        echo "Hidden=true" >> "$AUTOSTART_DIR/$item.desktop"
    fi
done
